...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    if __C.RESUME:
        ...
        if ('ckpt_' + __C.TASK_NAME + '_' + __C.DATASET + '_' + __C.VERSION) not in os.listdir(__C.CKPTS_PATH):
            os.mkdir(__C.CKPTS_PATH + '/ckpt_' + __C.TASK_NAME + '_' + __C.DATASET + '_' + __C.VERSION)
    ...
    logfile = open(
        __C.LOG_PATH +
        '/log_run_' + __C.TASK_NAME + '_' + __C.DATASET + '_' + __C.VERSION + '.txt',
        'a+'
    )
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            if __C.VERBOSE:
                ...
                print("\r[Task %s][Version %s][Model %s][Dataset %s][Epoch %2d][Step %4d/%4d][%s] Loss: %.4f, Lr: %.2e" % (
                    __C.TASK_NAME,
                    __C.VERSION,
                    __C.MODEL_USE,
                    __C.DATASET,
        ...
        torch.save(
            state,
            __C.CKPTS_PATH +
            '/ckpt_' + __C.TASK_NAME + '_' + __C.DATASET + '_' + __C.VERSION +
